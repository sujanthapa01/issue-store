...
export default function RepoPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                ...
                const onlyIssues = issuesRes.data.filter(
    ...
    return (
        <div className='max-w-3xl mx-auto px-4 py-6'>
            ...
            {repoInfo && (
                <div className='mb-6'>
                    <div> <button onClick={() => router.back()}>{"< back"}</button> <h1 className='text-2xl font-semibold'>{repoInfo.full_name}</h1></div>
                    <p className='text-sm text-gray-600'>{repoInfo.description}</p>
                    <div className='text-xs text-gray-500 mt-2 flex gap-3'>
                        ⭐stars - {repoInfo.stargazers_count} • 🍴Forks - {repoInfo.forks_count} • 🧑‍💻 {repoInfo.language}
                    </div>
                </div>
            )}
